...
class PublicEngagementServiceNew:
    ...
    async def get_monthly_trends(self) -> List[Dict[str, Any]]:
        """Calcule les tendances mensuelles (chaque mois indépendant)"""
        entries = await self.get_all_engagement_entries()
        ...
        for entry in entries:
            ...
            if "/" in date_str:
                ...
                if len(parts) == 3:
                    ...
                    monthly_data[month_key]["entries"].append(entry)
                    monthly_data[month_key]["total_score"] += entry["score"]
                    monthly_data[month_key]["total_vues"] += entry["vues"]
                    monthly_data[month_key]["total_likes"] += entry["likes_reactions"]
                    monthly_data[month_key]["total_partages"] += entry["partages"]
                    monthly_data[month_key]["total_commentaires"] += entry["commentaires"]
                    monthly_data[month_key]["total_abonnes"] += entry["nouveaux_abonnes"]
        
        # Convertir en liste et trier par mois
        trends = []
        for month_key, data in monthly_data.items():
            # Calculer la moyenne du score pour le mois
            avg_score = data["total_score"] / len(data["entries"]) if data["entries"] else 0
            
            # Extraire le mois pour l'affichage
            month_parts = month_key.split("/")
            month_num = month_parts[0]
            month_names = {
            ...
            trend_entry = {
                ...
                "total_partages": data["total_partages"],
        ...
        logger.info(f"✅ {len(trends)} tendances mensuelles calculées")
        return trends
